...
function getWeather(url) {
  $.getJSON(url, function (data) {
    ...
    document.querySelector(".top.first h3").textContent =
      weather.currently.summary;
    document
      .querySelector(".top.first i")
      .classList.add(weatherIcons[data.currently.icon]);
    document.querySelector(".top .temp").innerHTML =
      temp + '<i class="icon wi wi-' + unit + '"></i>';
    document.querySelector(".rain").innerHTML =
      data.currently.precipProbability.toFixed(0) * 100 +
    ...
    $.getJSON(
      `https://api.opencagedata.com/geocode/v1/json?q=${data.latitude}+${data.longitude}&key=14b2d2e206d945e6931169bc7e1ed01c`,
      function (data) {
        var searchLoc = $(".loc-search").val();
        if (searchLoc == "") {
          $(".loc-search").val(data.results[0].components.city);
        }
      }
    );
  });
...
function changeUnit24Today() {
  today.forEach(function (hour) {
    console.log(hour);
    if (unit === "fahrenheit") {
      hour[2] = Math.round(hour[2] * 1.8 + 32); // convert c to f
    } else {
      hour[2] = Math.round((hour[2] - 32) / 1.8);
    }
  });
...
bottomSections.forEach(function (section) {
  section.addEventListener("click", function () {
    if (document.querySelector(".bottom-expand") === null) {
      // check if any section expanded
      clearContent();
...
function displayFive() {
  bottomSections.forEach(function (section, i) {
    ...
    section.querySelector(".daytemp").innerHTML =
      fivedays[i][1] + '<i class="icon wi wi-' + unit + '"></i>';
    section.querySelector(".type").innerHTML =
      '<i class="icon wi ' + weatherIcons[fivedays[i][2]] + '"></i>';
...
function make5day() {
  ...
  for (var i = 0; i < 5; i++) {
    ...
    var daytemp = Math.round(
      (5 / 9) * (weather.daily.data[i + 1].temperatureMax - 32)
    );
...
function make24hour() {
  ...
  while (weather.hourly.data[i] != null) {
    ...
    var hourtemp = Math.round(
      (5 / 9) * (weather.hourly.data[i].apparentTemperature - 32)
    );
...
function makeTodaysHourly(hourly) {
  ...
  var i = 0;
  if (hourly[0][0] % 2 != 0) {
    // if showing limited, ensure starts on an even hour
    var i = 1;
  }
  for (var j = 0; j < 12; j++) {
...
function displayTodaysHourly(today) {
  ...
  today.forEach(function (hour) {
    ...
    hoursHtml +=
      "<div>" + zeroPadTime(hour[0]) + ":" + zeroPadTime(hour[1]) + "</div>";
    hoursHtml +=
      "<div>" + hour[2] + '<i class="icon wi wi-' + unit + '"></i></div>';
